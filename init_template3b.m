function [y]=init_template3b(nx,ny,nz,B)
n=nx*ny*nz;
mat=zeros(n,6);
for i=1:nx
    for j=1:ny
        for k=1:nz
            ...
            v1=sqrt(xx^2+yy^2);
            v2=(i/nx)^2/((0.5-epsilon)^2)+(j/ny)^2/((0.47-epsilon)^2);
            v3=(i/nx)^2/((0.5+epsilon)^2)+(j/ny)^2/((0.47+epsilon)^2);
  
          if (abs(k/nz-0.5)<epsilon)&&(v2>1)&&(v3<1)
            myv=[yy/v1 xx/v1 0; -xx/v1 yy/v1 0; 0 0 1];  
            mym=myv*diag([10,9,8])*myv';
