...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Simulation Example Code for time-dependent curves (H2)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
q0=1;
y_array{q0,1}=init_template1b(nx,ny,nz,B); % generate the response vector (under the null hypothesis)
DD_array{q0,1}=dtilda(y_array{q0,1},B);  % ols of the diffusion tensor D 

q0=2;
y_array{q0,1}=init_template3b(nx,ny,nz,B); % generate the response vector (under the alternative hypothesis 1)
DD_array{q0,1}=dtilda(y_array{q0,1},B);  % ols of the diffusion tensor D 
...
[U S V]=svds(P,2); % tsvd
chi_alt2(nsim,1)=diff'*pinv(U*S*V')*diff;
...
sum(chi_alt2>0) % it should be 100 since the values of the chi-square test cannot be negative.
sum(chi_alt2>chi2inv(0.95,2))/100 % power 
